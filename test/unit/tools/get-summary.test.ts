...
describe('get_summary tool', () => {
  ...
  it('returns a summary for valid location', async () => {
    const result = await handleGetSummary(service, { locationName });
  ...
  it('includes rating distribution', async () => {
    const result = await handleGetSummary(service, { locationName, period: '90d' });
  ...
  it('includes trend information', async () => {
    const result = await handleGetSummary(service, { locationName, period: '30d' });
    const structured = result.structuredContent as {
